var TAX_INVOICE_OCR_GROUP = __('Tax Invoice OCR');
	...
	async refresh(frm) {
		...
		if (frm.doc.verification_status === 'Needs Review' && frm.doc.fp_no) {
			frm.add_custom_button(__('✅ Approve Manually'), async () => {
                ...
                // 🆕 Create Purchase Invoice from OCR
                if (!frm.is_new() && frm.doc.dpp && frm.doc.verification_status === "Verified") {
                        frm.add_custom_button(__("🧾 Create Purchase Invoice"), async () => {
                                const dialog = new frappe.ui.Dialog({
                                        ...
                                        primary_action: async (values) => {
                                                ...
                                                if (r.message) {
                                                        frappe.show_alert({ message: __("✅ {0} created!", [r.message.purchase_invoice]), indicator: "green" }, 5);
