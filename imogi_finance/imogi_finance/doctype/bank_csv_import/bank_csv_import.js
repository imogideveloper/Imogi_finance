// Same handlers as bank_statement.js (loaded when doctype is still Bank CSV Import).
frappe.ui.form.on('Bank Statement', {
	refresh(frm) {
		setup_bank_statement_buttons(frm);
	},
	bank(frm) {
		setup_bank_account_query(frm);
	},
});
...
function setup_bank_statement_buttons(frm) {
	if (!frm.page) return;

	frm.remove_custom_button(__('Import CSV'));
	frm.remove_custom_button(__('Auto Reconcile Statement Detail'), __('Actions'));
	frm.remove_custom_button(__('Reset Reconcile'), __('Actions'));

	if (frm.doc.status !== 'Processing') {
		frm.add_custom_button(__('Import CSV'), () => run_bank_csv_import(frm)).addClass('btn-primary');
	}

	if (frm.doc.status === 'Completed') {
		frm.page.set_indicator(__('Completed'), 'green');
		frm.add_custom_button(__('Auto Reconcile Statement Detail'), () => auto_reconcile_statement(frm), __('Actions'));
		frm.add_custom_button(__('Reset Reconcile'), () => reset_reconcile_statement(frm), __('Actions'));
...
function setup_bank_account_query(frm) {
	if (!frm.doc.bank) return;
	frappe.db.get_value('Bank Statement Bank List', frm.doc.bank, 'bank').then((r) => {
		if (r.message && r.message.bank) {
			frm.set_query('bank_account', () => ({ filters: { bank: r.message.bank } }));
		}
	});
}

function run_bank_csv_import(frm) {
	if (!frm.doc.bank) {
		frappe.msgprint(__('Pilih Bank terlebih dahulu.'));
		return;
	}
	if (!frm.doc.bank_account) {
		frappe.msgprint(__('Pilih Bank Account terlebih dahulu.'));
		return;
	}
	if (!frm.doc.import_file) {
		frappe.msgprint(__('Upload file CSV terlebih dahulu.'));
		return;
	}
	if (frm.is_new()) {
	...
	frappe.confirm(__('Mulai import CSV dari bank {0}?', [frm.doc.bank]), () => {
		...
		frappe.call({
			...
			callback(r) {
				...
				const res = r.message;
				frappe.show_alert({
					message: __('Import selesai: {0} dibuat, {1} duplikat, {2} error', [
						res.created,
						res.skipped,
						res.errors,
					]),
...
function auto_reconcile_statement(frm) {
	frappe.call({
...
function reset_reconcile_statement(frm) {
	frappe.confirm(__('Reset semua status reconcile pada Statement Detail?'), () => {
		frappe.call({
