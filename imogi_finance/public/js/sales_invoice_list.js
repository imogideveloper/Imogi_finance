...
frappe.listview_settings["Sales Invoice"] = {
	...
	formatters: {
		...
		imogi_late_days(_value, _df, doc) {
			const late = cint(doc.imogi_late_days) || siDaysPastDue(doc) || 0;
			if (late > 0) {
				const clock =
					typeof frappe.utils.icon === "function"
						? frappe.utils.icon(
								"es-solid-alert-triangle",
								"xs",
								"",
								"",
								"si-late-icon si-late-icon-blink"
						  )
						: "";
				return (
	...
	onload(listview) {
		...
		init_sales_invoice_list_toolbar(listview);
	},
};
	...
	frappe.after_ajax(function () {
		if (!injectToolbar()) {
			...
			const interval = setInterval(function () {
				attempts += 1;
				if (injectToolbar() || attempts > 20) clearInterval(interval);
			}, 200);
		}
		scheduleApply(700);
	});
