// Sales Invoice list — Group By Period + Group By Status (terpisah)

function imogi_si_list_route_str() {
	try {
...
function isDocPastDue(doc) {
	...
	const late = cint(doc.imogi_late_days) || siDaysPastDue(doc) || 0;
...
const SI_STATUS_ICON_BY_STATUS = {
...
const SI_GROUP_ICON_BY_LABEL = {
	Unpaid: "es-solid-alert-circle",
	"Partly Paid": "es-solid-dot",
	Paid: "es-solid-success",
};

function siStatusKeyFromDoc(doc) {
...
function siStatusIconHtmlForDoc(doc) {
	const icon = SI_STATUS_ICON_BY_STATUS[siStatusKeyFromDoc(doc)] || "es-line-status";
	if (typeof frappe.utils.icon === "function") {
		return frappe.utils.icon(icon, "xs", "", "", "si-status-icon");
	}
	return `<svg class="icon icon-xs si-status-icon" aria-hidden="true"><use href="#${icon}"></use></svg>`;
}

function siGroupIconWrapHtml(label) {
	const icon = SI_GROUP_ICON_BY_LABEL[label] || "es-line-status";
	let svg = "";
	if (typeof frappe.utils.icon === "function") {
...
function siCountBadgeHtml(count) {
...
frappe.listview_settings["Sales Invoice"] = {
	...
	get_indicator: imogi_si_list_get_indicator,
	get_indicator_html: imogi_si_list_get_indicator_html,

	formatters: {
		status(_value, _df, doc) {
			return imogi_si_list_get_indicator_html(doc, false);
		},
		customer_name(value, df, doc) {
			const name = doc.name || "";
			const customer = value || doc.customer || "";
			return (
				`<span class="si-subject-cell">` +
					`<span class="si-subject-id">${frappe.utils.escape_html(name)}</span>` +
					(customer
						? `<span class="si-subject-customer text-muted"> · ${frappe.utils.escape_html(customer)}</span>`
						: "") +
				`</span>`
			);
		},
		posting_date: imogi_si_format_posting_date,
		grand_total: imogi_si_format_grand_total,
...
function imogi_si_list_onload(listview) {
...
function patch_sales_invoice_listview_settings() {
	...
	settings.get_indicator = imogi_si_list_get_indicator;
	settings.get_indicator_html = imogi_si_list_get_indicator_html;

	settings.formatters = settings.formatters || {};
	if (!settings.formatters.customer_name) {
		settings.formatters.customer_name = function (value, df, doc) {
			const name = doc.name || "";
			const customer = value || doc.customer || "";
			return (
				`<span class="si-subject-cell">` +
	...
	if (!settings.__si_imogi_onload_wrapped) {
		...
		settings.onload = function (listview) {
			...
			imogi_si_list_onload(listview);
			if (settings.get_indicator_html) {
				listview.get_indicator_html = settings.get_indicator_html.bind(listview);
			}
			imogi_si_list_ensure_toolbar(listview);
			if (orig_onload && orig_onload !== settings.onload) {
				orig_onload.call(this, listview);
			}
			imogi_si_list_ensure_toolbar(listview);
		};
		settings.__si_imogi_onload_wrapped = true;
	}
}

function imogi_si_list_get_indicator(doc) {
	const status = doc.status || "Draft";
	if (status === "Overdue" || status === "Overdue and Discounted") {
	...
	return [label, SI_STATUS_COLORS[status] || "grey", `status,=,${status}`];
}

function imogi_si_list_get_indicator_html(doc, show_workflow_state) {
	const indicator = frappe.get_indicator(doc, "Sales Invoice", show_workflow_state);
	if (!indicator) return "";
	...
	const statusKey = siStatusKeyFromDoc(doc);

	return (
		`<span class="indicator-pill si-status-pill ${color} filterable no-indicator-dot ellipsis"` +
			` data-filter="${frappe.utils.escape_html(filter)}" data-si-status="${frappe.utils.escape_html(statusKey)}" title="${title}">` +
			siStatusIconHtmlForDoc(doc) +
			`<span class="si-status-label">${frappe.utils.escape_html(label)}</span>` +
		`</span>`
	);
}

function imogi_si_format_posting_date(value) {
	if (!value) {
		return `<span class="si-cell-muted">—</span>`;
	}
	return `<span class="si-posting-date">${frappe.datetime.str_to_user(value)}</span>`;
}

function imogi_si_format_grand_total(value, df, doc) {
	if (value == null || value === "") {
		return `<span class="si-cell-muted">—</span>`;
	}
	const formatted = frappe.format(value, df, doc);
	return `<span class="si-grand-total">${formatted}</span>`;
}

function imogi_si_format_late_days(_value, _df, doc) {
	const late = cint(doc.imogi_late_days) || siDaysPastDue(doc) || 0;
	if (late > 0) {
		const clock =
			typeof frappe.utils.icon === "function"
				? frappe.utils.icon("es-solid-alert-triangle", "xs", "", "", "si-late-icon si-late-icon-blink")
				: "";
		return (
			`<span class="si-late-days-cell si-late-overdue text-danger" title="${frappe.utils.escape_html(
				__("Days past due date")
			)}">` +
				`<span class="si-late-icon-wrap si-late-icon-blink-wrap" aria-hidden="true">${clock}</span>` +
				`<span class="si-late-text">${late} ${__("Days Ago")}</span>` +
			`</span>`
		);
	}
	if (doc.due_date) {
		return `<span class="si-late-days-cell si-due-date">${frappe.datetime.str_to_user(doc.due_date)}</span>`;
	}
	return `<span class="si-late-days-cell si-cell-muted">—</span>`;
}

function imogi_si_list_ensure_toolbar(listview) {
	if (!listview || listview.doctype !== "Sales Invoice") return;
	window.__imogi_si_toolbar_active = true;
	if (typeof window.init_sales_invoice_list_toolbar === "function") {
		window.init_sales_invoice_list_toolbar(listview);
	}
}

// Client Script (__custom_list_js) runs AFTER this file and replaces listview_settings.
(function hook_si_list_after_client_script() {
	if (frappe.model.__imogi_si_list_meta_hooked) return;
	frappe.model.__imogi_si_list_meta_hooked = true;
	...
	frappe.model.init_doctype = function (doctype) {
		...
		patch_sales_invoice_listview_settings();
		setTimeout(() => {
			if (cur_list?.doctype === "Sales Invoice") {
				imogi_si_list_ensure_toolbar(cur_list);
			}
		}, 50);
	};
})();
...
if (typeof frappe.ready === "function") {
	frappe.ready(patch_sales_invoice_listview_settings);
} else {
	$(patch_sales_invoice_listview_settings);
}
frappe.after_ajax(() => {
	if (imogi_si_list_route_str() !== "List/Sales Invoice") return;
	patch_sales_invoice_listview_settings();
	imogi_si_list_ensure_toolbar(cur_list);
});
$(document).on("page-change", function () {
	if (imogi_si_list_route_str() === "List/Sales Invoice") {
		setTimeout(() => {
			patch_sales_invoice_listview_settings();
			imogi_si_list_ensure_toolbar(cur_list);
		}, 0);
...
function init_sales_invoice_list_toolbar(listview) {
	if (!listview || listview.doctype !== "Sales Invoice") return;
	...
	try {
		status_group_on = localStorage.getItem(STORAGE_KEY_GROUP) === "1";
	} catch (e) {
...
window.init_sales_invoice_list_toolbar = init_sales_invoice_list_toolbar;
